...
public class Runner {
    ...
    public void run(int blackCount, int blueCount, int whiteCount) throws InterruptedException {
        ...
        CountDownLatch blackL = new CountDownLatch(blackCount);
        CountDownLatch blueL = new CountDownLatch(blueCount);
        CountDownLatch whiteL = new CountDownLatch(whiteCount);

        for (int i = 0; i < blackCount; i++) {
            BlackThread blackThread = new BlackThread(blackL);
        ...
        for (int i = 0; i < blueCount; i++) {
            BlueThread blueThread = new BlueThread(blueL);
        ...
        for (int i = 0; i < whiteCount; i++) {
            WhiteThread whiteThread = new WhiteThread(whiteL);
